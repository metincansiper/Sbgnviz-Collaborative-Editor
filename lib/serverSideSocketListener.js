    ...
    var listenToAgentRequests = function(socket){
        ...
        socket.on('agentConnectToTripsRequest', function(){
            ...
            tm.init(function(){
                ...
                tm.sendMsg('(register :name bsb)');
                tm.sendMsg('(subscribe :content (tell &key :content (spoken . *)))');
                tm.sendMsg('(subscribe :content (tell &key :content (display-model . *)))');
                tm.sendMsg('(subscribe :content (tell &key :content (display-image . *)))');
                tm.sendMsg('(tell :content (module-status ready))');
                tm.sendMsg('(tell :content (start-conversation))');

            });

            // client.connect(6200, '127.0.0.1', function() {
            //     console.log('Connected to trips');
            //     client.write('(register :name bsb)');
            //     client.write('(subscribe :content (tell &key :content (spoken . *)))');
            //     client.write('(subscribe :content (tell &key :content (display-model . *)))');
            //     client.write('(subscribe :content (tell &key :content (display-image . *)))');
            //     client.write('(tell :content (module-status ready))');
            //     client.write('(tell :content (start-conversation))');
            // });
            //
            //
            // //data from trips
            ...
            //     var kqmlArr = kqml.parse(data.toString());
            //     var keywords = kqml.keywordify(kqmlArr);
            //
            ...
            //
            //     if(content && content[0]==='SPOKEN' && content[1] === ':WHAT')
            //         msg.comment = content[2];
            //     model.add('documents.' + msg.room + '.messages', msg);
            //
            //
            //
            //     //socket.emit("tripsMessage", data.toString());
            //     //io.in(socket.room).emit('message', msg);
            // });
