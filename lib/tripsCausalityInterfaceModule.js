...
module.exports = function(socket, model){
    ...
    this.makeIndraJson = function(causality){
        ...
        if(causality.rel.indexOf("PHOSPHORYLATES")>=0) {
        ...
        else if(causality.rel.indexOf("IS-PHOSPHORYLATED-BY")>=0) {//phosphorylation but with different source and targets
        ...
        else if(causality.rel.indexOf("DEPHOSPHORYLATES")>=0) {
        ...
        else if(causality.rel.indexOf("IS-DEPHOSPHORYLATED-BY")>=0) {//phosphorylation but with different source and targets
        ...
        else if(causality.rel.indexOf("UPREGULATES-EXPRESSION")>=0) {
            indraJson = { type: 'IncreaseAmount',  subj: {name: causality.id1,mods: [{ mod_type: 'phosphorylation', is_modified: true, residue: causality.res1, position: causality.pos1}]},
                obj: {name: causality.id2},residue: causality.res2, position:causality.pos2};
        }
        else if(causality.rel.indexOf("EXPRESSION-IS-UPREGULATED-BY")>=0) {//phosphorylation but with different source and targets
            indraJson = { type: 'IncreaseAmount',  subj: {name: causality.id2,mods: [{ mod_type: 'phosphorylation', is_modified: true, residue: causality.res2, position: causality.pos2}]},
                obj: { name: causality.id1},residue: causality.res1, position:causality.pos1};
        }
        else if(causality.rel.indexOf("DOWNREGULATES-EXPRESSION")>=0) {
            indraJson = { type: 'DecreaseAmount',  subj: {name: causality.id1,mods: [{ mod_type: 'phosphorylation', is_modified: true, residue: causality.res1, position: causality.pos1}]},
                obj: {name: causality.id2},residue: causality.res2, position:causality.pos2};
        }
        else if(causality.rel.indexOf("EXPRESSION-IS-DOWNREGULATED-BY")>=0) {//phosphorylation but with different source and targets
            indraJson = { type: 'DecreaseAmount',  subj: {name: causality.id2,mods: [{ mod_type: 'phosphorylation', is_modified: true, residue: causality.res2, position: causality.pos2}]},
                obj: { name: causality.id1},residue: causality.res1, position:causality.pos1};
        }
    ...
    tm.init(function(){
        ...
        tm.addHandler(pattern, function (text) { //listen to requests
            ...
            var source = self.getAttributeFromXML("name=",2,  text.content[2]);
           // var param = {source: text.content[2]};

            //Request this information from the causalityAgent
            socket.emit('findCorrelation', source, function(data){

                var msg = {userName: socket.userName, userId: socket.userId, room: socket.room, date: +(new Date)};
                msg.comment = source + ' '+ data.pSite1 + ' ' + data.id2 + ' ' + data.pSite2 + ' ' + data.correlation;
                model.add('documents.' + msg.room + '.messages', msg);
